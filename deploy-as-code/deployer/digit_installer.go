...
func main() {
	...
	if product != "" {
		...
		//if version != "" {
		argFile := releaseChartDir + product + "/dependancy_chart-" + "digit-v2.9" + ".yaml"
		...
		if selectedMod != nil {
			...
			var goDeployCmd string = fmt.Sprintf("go run main.go deploy -c -e central-instance %s", argStr)
