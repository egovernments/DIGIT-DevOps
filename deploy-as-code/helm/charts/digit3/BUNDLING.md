...
| Remove a service | delete its manifest entry → regenerate (that *is* the peel-apart, jar-side; the full deployment procedure — overrides, tests, images, chart, kong — is INSTALL.md §7, executed for billing on branch `modulith-separate-billing`) |
