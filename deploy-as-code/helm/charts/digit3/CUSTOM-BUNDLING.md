...
`05-build.sh --shape` / `06-deploy.sh --shape` **do not** cover a custom
...
Copy `digit3services-domain-bundles-helmfile.yaml` to
...
data, and the per-service shape's `postgres` DB is a separate dataset.
