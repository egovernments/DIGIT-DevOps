...
func main() {
	...
	infraType := []string{"1. Pilot/POC (Just for a POC where for a Quickstart and explore", 
	...
	var number_of_worker_nodes int = 0
	...
	cloudPlatforms := []string{"AWS", "AZURE", "GOOGLE", "On-prem/Private Cloud"}
	var optedCloud string = ""
	cloudPlatform, _ = sel(cloudPlatforms, "Choose the cloud type to privision the required servers for the DIGIT setup?")
	...
	case optedCloud == "AWS":
		...
		fmt.Println("Great, you need your" + cloudPlatform + "credentials to provision the cloud resources ..\n")
	...
	case optedCloud == "GOOGLE":
		fmt.Println("Great, you need your" + cloudPlatform + "credentials to provision the cloud resources ..\n")
		fmt.Println("Support for the " + cloudPlatform + "is still underway ... you need to wait")
...
func deployScript() {
	...
	if contextset {
		...
		if product != "" {
			files, err := ioutil.ReadDir("../helm/product-release-charts/" + product)
			if err != nil {
				log.Fatal(err)
			}
			...
			if version != "" {
				argFile := "../helm/product-release-charts/" + product + "/dependancy_chart-" + version + ".yaml"
